...
### 工作机制：
借用了本地`docker daemon`中配置的`registry-mirrors`加速功能，当我们利用`registry-mirrors`列表中的加速源，快速的把一些很难下载的镜像下载到本地后，再自动化的把镜像推送到云端（自己的私有镜像仓库中）保存起来。

同时处理了一些整个过程中的琐事，实现一键全自动化的完成`拉取->推送`
